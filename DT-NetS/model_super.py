...
def layer_evaluation(input_layer, weights, biases):
    ...
    h_p1 = tf.add(tf.nn.conv2d(h_current, weights[8], strides=[1, 1, 1, 1], padding='SAME'), biases[8])
    h.append(h_p1)
...
def forward(input_shape):
    ...
    flowU = tf.expand_dims(flow[:,:,:,0], axis=3)
    
    
    smooth_loss = compute_smooth_loss(flowU)
           
    return {'x1': x1, 'x2': x2, 'h' : h, 'W' : w, 'flow' : flow, 'smooth' : smooth_loss}
